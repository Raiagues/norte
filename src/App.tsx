...
type Route = "dashboard" | "invitations" | "newProject" | "admin" | "home" | "setup" | "teams" | "projectTeam" | "brainstorm" | "requirements" | "software" | "verification";
...
function getRoute(): Route {
  ...
  for (const [area, hash] of Object.entries(PROJECT_AREAS)) if (window.location.hash === hash) return area as Route;
...
export function App() {
  ...
  if (route === "dashboard") page = <TeamDashboardPage language={language} teams={teams} />;
  ...
  const discoveryAvailable = Boolean(activeProjectId) && !["dashboard", "invitations", "home", "teams", "setup", "newProject", "admin"].includes(route);
  const areaLabel = { dashboard: language === "pt" ? "Painel da equipe" : "Team dashboard", invitations: language === "pt" ? "Convites" : "Invitations", setup: language === "pt" ? "Memória do projeto" : "Project memory", projectTeam: language === "pt" ? "Equipe do projeto" : "Project team", brainstorm: language === "pt" ? "Concepção" : "Conception", requirements: language === "pt" ? "Requisitos" : "Requirements", software: "Software", verification: language === "pt" ? "Verificação" : "Verification", home: "", teams: "", newProject: "", admin: "" }[route];
  ...
  return (
    <div className={`${sidebarExpanded ? "app-shell sidebar-expanded" : "app-shell"} route-${route}${discovery.open && discoveryAvailable ? " discovery-open" : ""}`} style={discovery.open && discoveryAvailable ? { ["--discovery-width" as string]: `${discovery.width}px` } : undefined}>
      ...
      <div className="app-page"><AppHeader language={language} title={areaLabel || ({ home: language === "pt" ? "Início" : "Home", teams: language === "pt" ? "Equipes" : "Teams", newProject: language === "pt" ? "Novo projeto" : "New project", admin: language === "pt" ? "Administração" : "Administration" } as Record<string, string>)[route] || ""} onHome={route !== "home" ? openHome : undefined} onLanguageChange={changeLanguage} canManageTeams={teams.some(team => team.canManage)} />{auth.user?.testEnvironment && <div className="test-environment-banner">{language === "pt" ? "Ambiente de teste · cópia local dos dados" : "Test environment · local data copy"}</div>}{auth.user?.demoAccount && <div className="test-environment-banner">{language === "pt" ? "Conta de demonstração · equipe e projetos fictícios, apagados em 24 horas" : "Demonstration account · fictitious team and projects, deleted after 24 hours"}</div>}{saveError && <div className="save-error" role="alert">{language === "pt" ? "Alterações não salvas: " : "Unsaved changes: "}{saveError}<button type="button" onClick={flushProject}>{language === "pt" ? "Tentar novamente" : "Retry"}</button></div>}<div className="app-content">{page}</div></div>
