...
export function resolveInterpretation(model, text, output, language = "en") {
  ...
  const confirmation = typeof output.confirmation === "string" ? output.confirmation.trim() : "";
  const confirmable = confirmation && unclear(confirmation).question === confirmation;
  return { status: confirmable ? "confirmation" : "resolved", ...(confirmable ? { question: confirmation } : {}), summary: output.summary, change: { id: `change-${randomUUID()}`, targetEntityId: target.id, kind: output.kind, oldValues: structuredClone(target.properties), newValues: values, ...(output.kind === "replace_component" ? { replacementName: output.replacementName } : {}), description: text, createdAt: new Date().toISOString() } };
}
